...
docker-compose up -d
环境变量配置
application.yml 中的敏感信息或差异化配置通过环境变量注入。请根据你的实际值设置以下环境变量：
...
注意：所有带 ${...} 的配置项都必须通过环境变量提供，不要在 application.yml 中硬编码真实秘钥。

数据库表结构 (init.sql)
请将以下 SQL 语句完整保存为 init.sql：
...
运行应用
确保 Docker 环境已启动（MySQL、Redis、Qdrant）。

配置必要的环境变量（参考上面的表格）。

启动 Spring Boot 应用：

在 IDEA 中直接运行 Application 类。

或打包后运行：

bash
mvn clean package
java -jar target/pdf-ai-*.jar
访问接口（默认端口 8080），具体 API 文档见项目其他文档。

## 常见问题
# 1. Qdrant 连接失败？
检查 Qdrant 容器是否正常运行：docker ps | grep qdrant

确认应用配置中的 Qdrant 地址（默认 http://localhost:6333）

# 2. MySQL 时区错误？
在连接 URL 中已添加 serverTimezone=Asia/Shanghai，若仍有问题可修改容器时区。

#3. 环境变量不生效？
IDEA 中设置 Environment variables 的格式为 key=value;key2=value2

命令行使用 export 后再执行 java -jar

## 注意事项 ##
请勿将真实的 AccessKey/Secret 提交到 Git 仓库（已通过 .gitignore 忽略 *.yml，但仍需注意环境变量的安全）。

生产环境建议使用密钥管理服务（如 Aliyun KMS）或配置中心。

Redis 使用了 redis-stack-server，包含了 RediSearch 等模块，可用于向量辅助索引（可选）。
